    ...
    <>
      <div className="container my-2" style={{ color: props.myMode === 'dark' ? 'white' : '#042743' }}>
        ...
        <textarea
          ...
          rows="5"
        ></textarea>
        <button disabled={text.split(" ").filter((txt)=>{return txt.length}).length === 0} className="btn btn-primary mx-2 my-2 " onClick={setToUpperCase}>Convert To upperCase</button>
        <button disabled={text.split(" ").filter((txt)=>{return txt.length}).length === 0} className="btn btn-primary mx-2 " onClick={setToLowerCase}>Convert To LowerCase</button>
        <button disabled={text.split(" ").filter((txt)=>{return txt.length}).length === 0} type="submit" className="btn btn-primary mx-2 my-2" id="toggle" onClick={speak} >Speak</button>
        <button disabled={text.split(" ").filter((txt)=>{return txt.length}).length === 0}className="btn btn-primary mx-2 " onClick={removeSpace}>Remove Space</button>
        <button disabled={text.split(" ").filter((txt)=>{return txt.length}).length === 0}className="btn btn-primary mx-2 " onClick={copy}>Copy Text</button>
      ...
      <div className="container my-1" style={{ color: props.myMode === 'dark' ? 'white' : '#042743' }}>
        ...
        <p> {0.008 *text.split(" ").filter((txt) => { return txt.length !== 0 }).length} min, Average time to read words</p>
        <h3>Preview</h3>
